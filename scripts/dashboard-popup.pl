	...
	###### DemoDev02
			...
			else
			{
				...
				{
					$down_apps[$demodev02_ctr][9] = $app_name;
					$demodev02_ctr++;
				}
			}
	...
	#   $test_service_str = `cat apps_list.json | jq '.apps[]|"\\(.name) \\(.environment_json.ARTIFACT_VERSION)"' | awk '{print \$2}'`;
		...
		@service_count = `cat apps_list.json | jq ".apps[].service_count"`;
	...
	#        $sub_hash{$name1}[6] = $temp_bound_service[$i];
			...
			$sub_hash{$name1}[8] = $service_count[$i];
		
		if ($temp_running_instances[$i] eq 0)
		{
			$sub_hash{$name1}[9]="#ff0000";
		}
			elsif ($temp_running_instances[$i] ne $temp_total_instances[$i])
			{
				$sub_hash{$name1}[9]="#ffa8af";
			}
			else
			{
				$sub_hash{$name1}[9]="#e2f4ff";
			}
			
			#if (length($temp_bound_service[$i]) eq "undef")
			if (!defined $temp_bound_service[$i] || $temp_bound_service[$i] eq '')
			{
				$sub_hash{$name1}[6] = "No Service Bind";
